...
public class ActivityCheck extends FragmentActivity implements View.OnClickListener/*, View.OnLongClickListener, InputFragment.onSomeEventListener*/ {
    ...
    private class AutoWeightThread extends Thread {
        ...
        public void run() {
            ...
            while (!cancelled) {
                ...
                while (!cancelled && !isCapture() && !weightViewIsSwipe) {                                              //ждЄм начала нагружени€
                    try {Thread.sleep(50); } catch (InterruptedException ignored) { }
                }
                ...
                while (!cancelled && !(isStable || weightViewIsSwipe)) {                                                //ждем стабилизации веса или нажатием выбора веса
                    try { Thread.sleep(50); } catch (InterruptedException ignored) {}
                    if (!touchWeightView) {                                                                              //если не прикасаемс€ к индикатору тогда стабилизируем вес
                ...
                if (isStable) {
                    handler.sendMessage(handler.obtainMessage(ACTION_STORE_WEIGHTING, moduleWeight, 0));                 //сохран€ем стабильный вес
                }
                ...
                if (cancelled) {
                    if (isStable && weightType == WeightType.SECOND) {                                                  //≈сли тара зафоксирована и выход через кнопку назад
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setTitle(getString(R.string.input_check) + " є " + entryID + ' ' + ": " + values.getAsString(CheckDBAdapter.KEY_VENDOR)); //установить заголовок
    ...
    final HandlerWeightUpdate handlerWeight = new HandlerWeightUpdate() {
        ...
        public int handlerWeight(final HandlerWeightUpdate.Result what, final int weight, final int sensor) {
            ...
            return 20; // ќбновл€ем через милисикунды
        }
    };
}
